...
- **Skills**（model-invoked，安裝後到處可用）
  - `six-rung-ladder` —「到底該不該做」的極簡過濾網（decide）。
  - `agentic-sop` — 把人工流程工程化成自動代理工作流的方法論與落地入口（build）；含**拓撲宣告**步驟。
  - `agentic-workflow-audit` — 稽核既有 workflow 是否真拆解，或退化成 mega agent（audit）；七項檢查 ＋ 三項試金石。
- **Command**：`/agentic-sop-kit:sop-flow` — 跑 kit 的編排（線性 demo 或已宣告的拓撲），回報 DRAFT。
- **Hooks（專案範圍，關鍵設計）**
...
## 引擎能力（kit 的 `run.py` 編排器，v1.10）
除了線性 demo，引擎還支援以下能力——**全部確定性、由程式決定、附加式**（既有流程照常運作）：
- **每步硬閘門**（產出後驗、fail 即停、零 LLM）：`cmd_gate`（exit 0）／`schema_gate`（必填欄位；加 `schema_ref` 即**連 tag 與型別一起驗**＝有型別的邊）／`trace_gate`（值須 verbatim 溯源 → 擋臆造）／`recompute_gate`（數字重算相符）／`residual_gate`（必填欄位仍是【待補】即擋）。
- **`cmd` 步驟**（指令只來自 `flow.json` 白名單；會改動環境的需 `--allow-mutations`）＋ **`--plan`** 乾跑——列出所有操作、標示 mutating，**並靜態驗證整張拓撲**（不可達節點／read-before-write／寫入衝突／無界環／孤邊），不合法在跑之前就 exit 2。
- **條件分支**（`branch`／`cases`／`goto`）、**有界退回邊**（`back:true` ＋ `max_revisits`）與 **`map_over`**（對清單每項各跑一次、收集）——控制流由程式決定，不交給模型。
- **狀態宣告**（`reads`／`writes`）：每個欄位唯一 writer、讀之前所有路徑保證寫過；值仍留在 artifact（沒有第二份權威），欄位 owner 與實走路徑記在 manifest 的 `topology`／`path_taken`。
- **`--graph`** 畫出拓撲（Mermaid ＋ 文字），且該圖由測試綁住與 `flow.json` 一致。
- **跨領域範例**：`kit/workflow/examples/` 的免依賴 FE／BE／DB／AI 流程（各示範一個閘門），外加四節點、含真的有界退回邊的 `graph.json`。
...
- 共享狀態必須有契約（每個欄位唯一 writer、讀之前保證寫過、邊上產物有型別）；環必須有界。
- **真正的強制力在 hook/gate（本 plugin 的 `Stop` 閘門與 `--plan` 靜態拓撲閘門），不在散文。**
